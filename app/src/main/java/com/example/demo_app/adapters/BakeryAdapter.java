...
public class BakeryAdapter extends RecyclerView.Adapter<BakeryAdapter.ViewHolder> {
    ...
    public BakeryAdapter(Context context, List<BakeryModel> bakeryModelList) {
        this.context = context;
        this.bakeryModelList = bakeryModelList;
    }

    @NonNull
    @Override
    public BakeryAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.product_list_items,parent,false));
    }

    @Override
    public void onBindViewHolder(@NonNull BakeryAdapter.ViewHolder holder, @SuppressLint("RecyclerView") int position) {

        Glide.with(context).load(bakeryModelList.get(position).getImg_url()).into(holder.imageView);
        holder.name.setText(bakeryModelList.get(position).getName());
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                i.putExtra("type",bakeryModelList.get(position).getType());
